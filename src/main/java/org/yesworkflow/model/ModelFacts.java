...
public class ModelFacts {
	...
	private DataExportBuilder amountFacts;
	private Long nextLogId = 1L;

	private int fileAmount_total = 0;
	...
	public ModelFacts(QueryEngine queryEngine, Model model) throws IOException {
		...
		if (model.workflow == null)
			throw new IllegalArgumentException(
					"Null workflow field in model argument passed to ModelFacts constructor.");

		this.model = model;

		this.programFacts = DataExportBuilder.create(queryEngine, "program", "program_id", "program_name",
				"qualified_program_name", "begin_annotation_id", "end_annotation_id");
		...
		this.portFacts = DataExportBuilder.create(queryEngine, "port", "port_id", "port_type", "port_name",
				"qualified_port_name", "port_annotation_id", "data_id");
		...
		this.portConnectionFacts = DataExportBuilder.create(queryEngine, "port_connects_to_channel", "port_id",
				"channel_id");
		this.inflowConnectionFacts = DataExportBuilder.create(queryEngine, "inflow_connects_to_channel", "port_id",
				"channel_id");
		this.outflowConnectionFacts = DataExportBuilder.create(queryEngine, "outflow_connects_to_channel", "port_id",
				"channel_id");
		this.portUriVariableFacts = DataExportBuilder.create(queryEngine, "uri_variable", "uri_variable_id",
				"variable_name", "port_id");
		this.portLogFacts = DataExportBuilder.create(queryEngine, "log_template", "log_template_id", "port_id",
				"entry_template", "log_annotation_id");
		this.logTemplateVariableFacts = DataExportBuilder.create(queryEngine, "log_template_variable",
				"log_variable_id", "variable_name", "log_template_id");
		this.amountFacts = DataExportBuilder.create(queryEngine, "dataAmount", "port_id", "amount", "file_production");
	}
	...
	public ModelFacts build() throws IOException {
		...
		amountFacts.addRow("","","total amount of fileproduction: " + fileAmount_total);
		...
		facts.put(amountFacts.name, amountFacts.toString());
	...
	private void buildProgramFactsRecursively(Program program, String parentName, Long parentId) throws IOException {
		...
		programFacts.addRow(program.id, program.beginAnnotation.value(), qualifiedProgramName,
				program.beginAnnotation.id, program.endAnnotation.id);
	...
	private void buildPortFacts(Port[] ports, Program block, Long blockId, String programName) throws IOException {
		...
		for (Port port : ports) {
			...
			//fill amount modelfacts with the right data
			if (port.flowAnnotation.amountAnnotation() != null) {
				String amountValue = port.flowAnnotation.amountAnnotation().amount();
				if (amountValue != null) {
					if (port.flowAnnotation instanceof Out && port.uriTemplate != null) {
						amountFacts.addRow(port.id, amountValue, true);
						fileAmount_total += Integer.valueOf(amountValue);
					} else {
						amountFacts.addRow(port.id, amountValue, false);
					}
				}
			}
